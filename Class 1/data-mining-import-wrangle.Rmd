...
# Below are some helpful links for GitHub!
...
***NOTE: the 'r' in the curly bracket means that we will execute code using R. We can also specify this to Python or SQL if we want to code using those languages.

***NOTE: The 'setup' portion of the code chunk (see below) is normally what we use to install all the packages we're going to use in the Rmarkdown file in the beginning. Typically, we only see the 'setup' once, and it's usually at the start of the file, but if we're working with colleagues and want to introduce their attention to the new package, we can set up another 'setup' code chunk. But this isn't really necessary lest for training/teaching purposes.
...
# For reference, the Windows "r chunk" creation shortcut to create a code chunk is: Ctrl + alt + i


#Links to packages used for today's lecture below:

https://www.tidyverse.org/
https://readxl.tidyverse.org/ --> *This package is used to import excel spreadsheets
http://sfirke.github.io/janitor/articles/janitor.html --> *This package is used to clean up column names
https://lubridate.tidyverse.org/ --> *This package is used to work with dates
...
***NOTE: The functions listed above can be used to cover 90% of what's needed to do when WRANGLING DATA!
...
```{r}
read_excel("~/School/Courses/STAT 4310 - Statistical Data Mining/Class Notes/08.23.22 Lecture 1 - Data Import & Wrangling/Section1All_xls.xlsx", 
    sheet = "T10101-Q", skip = 7)
```
...
***NOTE: Not sure why I can't load the following images from my computer directory. Try again later? It's not that important, but it's something that could be useful when creating future RMarkdown Files!
...
*NOTE: The "read_excel()" code is from the readexcel library package we downloaded previously, and it's how we turn an excel spreadsheet into a dataframe, and the "curl_download()" is how we downloaded the excel dataset onto our computer
...
Unless we already know which one we want, we can view the available sheets with the `excel_sheets()` function from the `readxl` package (just run the "excel_sheets(destfile)" function to preview the sheets!). We pipe the results of `excel_sheets(destfile)` to the `tibble()` function so we can store the results in a data frame.

```{r}
library(readxl)

excel_sheets(destfile) %>% # the pipe operator: allows us to take DF and pass to a function and then pipe it to another function. In other words, it connects functions to other functions, so that way we don't have to create and assign a variable every time and we thus only need to save one object/variable!
  tibble() # is a dataframe, table, tbl, tibble
```

# For reference, the Windows "pipe operator" creation shortcut to create a code chunk is: Ctrl + Shift + m
...
###NOTE: Look into the "tidymodels" package, as a way for cleaner & more advanced modeling and machine learning using tidyverse principles.
...
read_excel(destfile, sheet = "T10101-Q", 
  ...
  pivot_longer(-account:-code, names_to = "date", values_to = "percent_change") %>% 
  mutate(date = parse_date_time(date, orders = "Yq")) %>% # When using the 'mutate' function, we whatever variable we call will be overwritten and changed. If we want to create a new variable/column and keep the original, we just need to assign a new variable object with whatever change we want to make, and we'll keep the original!
  head()
```
...
read_excel(destfile, sheet = "T10101-Q", 
  ...
  pivot_longer(-account:-code, names_to = "date", values_to = "percent_change") %>% 
  mutate(better_date = parse_date_time(date, orders = "Yq"), # Here we can just create a new column/variable called "better_date" and keep the original! We can keep creating variables through the same mutate function, as it's easier to stack instead of ceating a new mutate function.
         date = ymd(better_date)) %>% # ymd()is a function that just formats a date to "year, month, day". Alternatively, there are other format options like: "mdy()" or "dmy()".
  head()
```
...
read_excel(destfile, sheet = "T10101-Q", 
  ...
  filter(percent_change < 0, 
         account == "Gross domestic product") # The '==' means that we want to filter everything under the "GDP" sub-column. If we wanted to filter out "GDP", we would simply change the '==' sign to a '!=' sign. This works the same way in Python!
```
...
read_excel(destfile, sheet = "T10101-Q", 
  ...
  mutate(date = parse_date_time(date, orders = "Yq"),
         date = ymd(date)) %>%  
  group_by(account) %>% # The 'group_by()' function lets the DF know that each account is its own discrete thing.
  slice(1, n())
```

#After we run the above chunk of code, we see that under "Account" there are some labels called 'Addendum'. This is just some weird metadata that was downloaded when we imported the dataset, so we can filter that out by adding the 'filter()' function to the same code chunk. 
```{r}
...
read_excel(destfile, sheet = "T10101-Q", 
  ...
  mutate(date = parse_date_time(date, orders = "Yq"),
         date = ymd(date)) %>% 
  filter(account == "Gross domestic product")  %>% 
  arrange(desc(date)) %>% #This function 'arrange()' allows us to either arrange our data in ascending/descending format. Here, we're using descending for the "date" variable, but we could also specify it to arrange by the "percent_change" variable or whatever other variable!
  head()
```
...
read_excel(destfile, sheet = "T10101-Q", 
  ...
  filter(account == "Gross domestic product")  %>% 
  arrange(desc(date)) %>% 
  relocate(date, everything()) %>% # The 'relocate()' function allows us to relocate a column/variable to any positional order relative to the other columns. We can get really specific, but for this purpose, we're just moving "date" to the far left relative to every other column.
  head()
...
#Showing the usefulness of the 'distinct()' function:
...
read_excel(destfile, sheet = "T10101-Q", 
  ...
  distinct(account, .keep_all = T) # We can use the 'distinct()' function to tell us all the distinct values of any column. Here, we're using it for "account" variable! If we just input "account", then it shows us only the values for "account". If, however, we want to keep the other columns, we just use the argument, '.keep_all = T'. This keeps ALL the values that go with those unique  account names.
```


```{r}
gdp_percent_change_from_spreadsheet <- # Here, we are assigning all of the code we've used previously to tidy-up the DF to a new variable. We HAVE to do this, because so far, all of our "assignments" have been nested in functions, and the objects are not actually located outside of these functions. This works very similarly to Python!
 read_excel(destfile, sheet = "T10101-Q", 
    skip = 7) %>% 
  select(-1)  %>% 
  rename(account = 1, code = 2) %>%
...
***NOTE: data frame = table = tbl = tibble --> This naming convention is just a result of people being lazy, but they all mean the same
...
read_excel(destfile, sheet = "T10101-Q", 
  ...
  select(-code) %>% 
  pivot_wider(names_from = account, values_from = percent_change) # This function is the opposite of what we did with the 'pivot_longer()' function
```
...
Next we call the `curl_download()` function, supply our pasted string variable `market_structure_data_address` and point to the `temp` location. 
...
# The reason we create a temp location is because we may get a zip file, and we have no way of knowing what's actually in it. So we store the zip file in the temp location to play with before we actually use it!
...
file_name <- # If we just highlight the variable "file_name" and only run that bit of code, it will run the rest of the code assigned to it. It's a little faster than typing the variable into a new code_chunk to view the variable output!
  unzip(temp, list = TRUE) %>% 
  slice(1) %>% # This slices off the first row!
  pull(Name) #This function allows us to pull out a value that is in a column. In this case, we're pulling out "Name"!
```
...
q2_2021 %>% 
  group_by(security) %>%
  summarise(distinct_tickers = n_distinct(ticker)) #The 'summarise()' function allows us to summarize a column!
```
...
q2_2021 %>% 
  ...
  summarise(mean_vol = mean(order_vol_000),
            ...
            sd_vol = sd(order_vol_000))
```



Well, we could keep exploring this data set but let's think ahead to when we might need to do this again, or do so in a more scaleable way and create a function that we can call. 

# There's a package called dbplyr, that allows us to write dplyer code against a database. Basically, we can write SQL code by using RStudio!
